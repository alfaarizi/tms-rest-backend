...
class CronController extends BaseController
{
     ...
     */
    public function actionCheckSubmission()
    {
        ...
        $IDs = array_keys((new \yii\db\Query())
            ->select('id')
            ->from('{{%tasks}}')
            ->where(['autoTest' => 1])
        ...
        // Get the tasks with a Dockerfile uploaded
        /*$IDs = array_keys((new \yii\db\Query())->select('taskID')
            ->from('{{%instructorFiles%}}')
            ->where(['name' => 'Dockerfile', 'taskID' => $IDs])
            ->indexBy('taskID')
            ->all());*/
        ...
        // Else run the tests.
        $tester = new AssignmentTester($studentFile, $testCases, Yii::$app->params['evaluator'][$studentFile->task->testOS]);
        $tester->test();
