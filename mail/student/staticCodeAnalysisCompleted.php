...
<?php else : ?>
    <?php foreach ($codeCheckerResult->codeCheckerReports as $report) : ?>
        <?=
        MailHtml::table([
            ['th' => Yii::t('app/mail', 'File (line, column)'), 'td' => "$report->filePath ($report->line, $report->column)"],
            ['th' => Yii::t('app/mail', 'Checker'), 'td' => $report->checkerName],
            ['th' => Yii::t('app/mail', 'Severity'), 'td' =>  $report->severity],
            ['th' => Yii::t('app/mail', 'Category'), 'td' => $report->category],
            ['th' => Yii::t('app/mail', 'Message'), 'td' => $report->message]
        ])
        ?>
    <?php endforeach; ?>
<?php endif; ?>
